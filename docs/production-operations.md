...
```bash
old_image=45th-homepage:<old-commit-sha>
PAYLOAD_IMAGE="$old_image" docker compose \
  --env-file .env.production \
  --env-file .env.release \
  -f compose.prod.yml up -d --no-deps --wait --wait-timeout 120 payload
...
2. 新しい`main`へ更新するが、`prod:deploy`はまだ実行しない。
3. PayloadとTunnelを停止し、旧S3から統合backupを作る。
4. 旧SeaweedFSサービスを停止する。旧volumeは削除しない。
5. `seaweedfs-mini-data`という名前を含むvolumeが存在しないことを確認する。存在する場合は削除せず、内容と作成理由を確認する。
6. 新しい`weed mini`を起動し、backupのmediaをS3 API経由でuploadする。
7. object keyと容量の一覧を比較する。
