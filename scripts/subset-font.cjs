...
function extractCharsFromDir(dir) {
  const chars = new Set();
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (entry.name === "node_modules" || entry.name === ".next" || entry.name === "dist")
        continue;
      const subChars = extractCharsFromDir(fullPath);
...
async function main() {
  ...
  const extractedChars = extractCharsFromDir(srcDir);
  ...
  const text = BASIC_CHARS + Array.from(extractedChars).join("") + COMMON_KANJI;
