...
type PopupProps = Omit<ModalOverlayProps, "children"> & {
  surveyHref: string;
};
  ...
  return (
    ...
    >
      <Modal className="relative w-full max-w-84 outline-none md:max-w-200 entering:duration-200 entering:ease-out entering:animate-in entering:zoom-in-95 exiting:duration-150 exiting:ease-in exiting:animate-out exiting:zoom-out-95">
        ...
        <Dialog className="relative max-h-[calc(100dvh-var(--spacing-3l))] overflow-hidden rounded-2xl border-8 border-main bg-timetable-base-dark text-font-main shadow-[0_0_8px_rgba(60,224,232,0.7)] outline-none">
          <Image
            ...
            width={265}
            height={265}
            className="pointer-events-none absolute -top-10 -right-10 size-40 opacity-90 mix-blend-screen md:-top-20 md:-right-20 md:size-80"
          />
          ...
          <div className="max-h-[calc(100dvh-56px)] overflow-y-auto px-m pt-ll pb-l md:max-h-[calc(100dvh-var(--spacing-5l))] md:px-3l md:py-3l">
            ...
            <div className="mt-s flex w-full justify-center md:mt-l [&>a]:w-full [&>a]:max-w-55 [&>a]:px-s md:[&>a]:max-w-75 md:[&>a]:px-3l">
              <ButtonMain href={surveyHref} title="アンケートはこちらから" arrow={false} newTab />
            </div>
          </div>
        </Dialog>
      </Modal>
