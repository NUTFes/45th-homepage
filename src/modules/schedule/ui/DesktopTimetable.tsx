...
export default function DesktopTimetable({ groupModels, onLaneChange }: DesktopTimetableProps) {
  ...
  return (
    <section
      ...
      className="hidden border-y-2 border-main bg-base/60 px-pm pt-4l pb-4l md:block"
    >
      <div className="mx-auto max-w-260">
        <nav aria-label="会場グループ" className="mb-l [scrollbar-width:thin] overflow-x-auto">
            ...
            {groupModels.map(({ group }) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-12 min-w-36 cursor-pointer rounded-lg border-2 border-main px-m py-ss text-Pbutton font-bold outline-hidden transition-colors hover:bg-secondary/90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-main ${
                    isSelected ? "bg-white text-base-dark" : "bg-base-dark text-font-main"
                  }`}
                  key={group.id}
